<template>
    ...
    <swd-menu role="navigation">
		...
		<main>
		<RouterView/>
		<DialogComponent v-if="dialog.open.value" :modelValue="dialog.open.value !== undefined" :consent="dialog.open.value.consent" @update:modelValue="$event ? {} : dialog.open = undefined" :title="dialog.open.value.title" :action="dialog.open.value.action" :filter="dialog.open.value.filter" @success="success()">
...
if (!cookies.get('cookie-consent')) {
    dialog.open = {
        ...
        content: ['<p>Wir verwenden zum Betreiben der Seite technisch notwendige Cookies. Weitere informationen in der <a target="_blank" href="/privacy-policy">Datenschutzerklärung</a>.</p>'],
        action: 'Verstanden',
		consent: true,
        success: () => cookies.set('cookie-consent', 'true')
    }
}
